...
from ui_endplateR02 import Ui_MainWindow
...
#         outObj['Plate']["Moment Capacity (kN-m)"] = float(self.ui.txtMomntCapacity.text())
    ...
    def save_design(self,popup_summary):
        ...
        fileName,pat =QtGui.QFileDialog.getSaveFileNameAndFilter(self,"Save File As","output/","Html Files (*.html)")
        fileName = str(fileName)
        self.call2D_Drawing("All")
        print 'printing html file path', fileName
        print "popupsummary", popup_summary
        ...
        pdfkit.from_file(fileName,'output/endPlateReport.pdf')
        
        QtGui.QMessageBox.about(self,'Information',"Report Saved")
        
    def save_log(self):
        
        fileName,pat =QtGui.QFileDialog.getSaveFileNameAndFilter(self,"Save File As","output/SaveMessages.txt","Text files (*.txt)")
...
#         self.ui.btnSvgSave.setEnabled(False)
    ...
    def callDesired_View(self,fileName,view):
        ...
        dictbeamdata  = self.fetchBeamPara()
        dictcoldata = self.fetchColumnPara()
        endCommonObj = EndCommonData(uiObj,resultObj,dictbeamdata,dictcoldata)
...
def launchEndPlateController(osdagMainWindow):
    set_osdaglogger()
